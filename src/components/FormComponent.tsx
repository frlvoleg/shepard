...
function NestedFormComponent(props: any) {
  const [nestedAttrs] = useNestedConfigurator(props.address);

  if (!Object.values(nestedAttrs || {}).length) return <></>;

  return Object.values(nestedAttrs).map((attr: any) => {
    if (!(attr as any).values?.length) return null;

    let Component = ATTRIBUTE_TYPE_DEFAULTS[(attr as any).type];

    if (!Component) {
      console.log(
        `No default component available for ${(attr as any).type} type Attributes`
      );
      return <></>;
    }

    if ((attr as any).type !== 'Asset') return <Component attribute={attr} />;

    return (
      <>
        <Component attribute={attr} />
        <NestedFormComponent address={[...props.address, (attr as any).name]} />
      </>
    );
  });
}

function RootFormComponent(props: any) {
